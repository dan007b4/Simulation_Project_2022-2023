...
public class Personnel {
    ...
    private double[][] mu;                      // Processing rate per station and job type
    private double[][] var;                     // Variance per station and job type
    private double[][] sigma;                   // Standard deviation per station and job type
    private int nD;                             // Number of scans handled
    private int[] nDWS;                         // Number of scans handled in a particular workstation
    private double[][] tD;                      // Time of next departure for each server in each workstation
    ...
    private double[] meanServiceTime;           // Calculated average service time
    private double tMu;                         // Generated service time
    private double[] totMu;                     // Total service time generated
    private double[][][] timeService;           // Service time per customer and workstation
    private int[][] currentCust;                // Customer handles by a particular workstation and server
    private int[][] listScan;                   // List of customers processed at a particular workstation on a particular point in time

    /* PARAMETERS RELATED TO WAITING OF SCANS */
    private double[] meanWaitingTime;           // Calculated average waiting time per source
    private double[] waitingTime;               // Waiting time for a particular customer
    private double[][][] waitingTimeJobWS;      // Waiting time for a job on a particular workstation
    private double[] meanCustomersQueue;        // Calculated average number of scans in queue per source
    private double[] totNQueue;                 // Total number of scans in queue over time
    private double[][] totNQueueWS;             // Total number of scans in queue at workstation over time

    /* VARIABLES RELATED TO PROCESSED SCANS */
    private double[][] timeDeparture;           // Time of departure of the scan from the ancillary services
    private double[][][] timeDepartureWS;       // Time of departure of a particular scan from a particular workstation
    private double[][] timeSystem;              // Time in system for a particular customer
    private double[][][] timeSystemJobWS;       // Time in system for a particular job on a particular workstation
    private int[] orderOut;                     // Order in which jobs are coming out of the system
    private double[] meanSystemTime;            // Calculated average time in system per source

    /* OTHER PARAMETERS */
    private int infinity;                       // Value for infinity
    private double[][][] idle;                  // Idle time for server s at workstation w
    private double[][] rhoWSS;                  // Utilization of server s at workstation w
    private double[] rhoWS;                     // Utilization of workstation w
